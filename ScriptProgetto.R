...
plot_pas <-  ggplot(pas_ts, aes(x = anno, y= NordEst, color = "NE")) +
  ...
  scale_color_manual(name = "Regione",
                     ...
                     labels = c("Nord Est", "Nord Ovest", "Centro", "Sud", "Isole")) +
  guides(color = guide_legend(override.aes = list(size = 3))) +
  geom_text(aes(x = 2020, y = 6*10^8 , label = "Covid 19"), color = 'black', size = 4) +
...
NordEst_ts <- cbind(NordEst_ts, passeggeri = pas_ts$NordEst[1:20])
...
NordEst_ts <- cbind(NordEst_ts, movimenti = mov_ts$NordEst[1:20])
...
plot_mov <-  ggplot(mov_ts, aes(x = anno, y= NordEst, color = "NE")) +
  ...
  scale_color_manual(name = "Regione",
                     ...
                     labels = c("Nord Est", "Nord Ovest", "Centro", "Sud", "Isole")) +
  guides(color = guide_legend(override.aes = list(size = 3))) +
  geom_text(aes(x = 2020 , y=5*10^6, label = "Covid 19"), color = 'black', size = 4) +
...
NordEst_ts <- rbind(NordEst_ts, c(2020, Tur_ts[21,2], mov_ts[21,2], pas_ts[21,2]))
...
plot_pil <-  ggplot(Pil_ts, aes(x = anno, y= NordEst, color = "NE")) +
  ...
  scale_color_manual(name = "Regione",
                     ...
                     labels = c("Nord Est", "Nord Ovest", "Centro", "Sud", "Isole")) +
  guides(color = guide_legend(override.aes = list(size = 3))) +
  geom_text(aes(x = 2020, y = 6.2*10^5 , label = "Covid 19"), color = 'black', size = 4) +
...
plot_spe <- ggplot(SpesaRegione_ts, aes(x = anno, y= NordEst, color = "NE")) +
  ...
  scale_color_manual(name = "Regione",
                     ...
                     labels = c("Nord Est", "Nord Ovest", "Centro", "Sud", "Isole")) +
  guides(color = guide_legend(override.aes = list(size = 3))) +
  geom_text(aes(x = 2020, y = 2200 , label = "Covid 19"), color = 'black', size = 4) +
...
PopPasTimeModel_NE <-lm(turismo ~ anno + passeggeri + popolazione -1, data = NordEst_ts)
# Vediamo il summary
...
plot_PopPasTimeModel_NE <- ggplot(NordEst_ts, aes(x=anno, y=turismo)) +
  ...
  ggtitle("Tasso di turisticità nel Nord-Est") +
  theme_classic()
# Decido di scartare la variabile popolazione
# Per il confronto in serie storica non è tornata utile
# Probabilmente mi servirà per il confronto longitudinale
